...
const Habits = () => {
  ...
  const fetchEntries = useCallback(async (habitId) => {
    const start = format(subDays(new Date(), 60), 'yyyy-MM-dd');
    const end = format(new Date(), 'yyyy-MM-dd');
    const { data } = await habitAPI.getEntries(habitId, { start, end });
    return data.data;
  }, []);
  ...
  useEffect(() => {
    if (habits.length) {
      Promise.all(habits.map(async (h) => {
        const e = await fetchEntries(h._id);
        return [h._id, e];
      })).then((results) => {
        const map = {};
        results.forEach(([id, e]) => { map[id] = e; });
        setEntries(map);
      });
    }
  }, [habits, fetchEntries]);
  ...
  const toggleHabit = async (habit, completed, value = 0) => {
    try {
      ...
      setEntries((prev) => {
        ...
        const idx = habitEntries.findIndex((e) => format(new Date(e.date), 'yyyy-MM-dd') === format(selectedDate, 'yyyy-MM-dd'));
        if (idx >= 0) habitEntries[idx] = data.data;
        else habitEntries.push(data.data);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.layout}>
        <div className={styles.habitList}>
          ...
          {habits.map((habit) => (
            <Card key={habit._id} className={styles.habitCard}>
              <div className={styles.habitHeader}>
                ...
                <div className={styles.habitActions}>
                  <button onClick={() => openEdit(habit)} className={styles.editBtn}>Edit</button>
                  <button onClick={() => handleDelete(habit._id)} className={styles.deleteBtn}>×</button>
                </div>
              </div>
              <ProgressBar value={habit.completionRate || 0} label="30-day completion" color={habit.color} size="sm" />
              {habit.type === 'yes_no' ? (
                <button
                  className={`${styles.toggleBtn} ${isCompleted(habit._id) ? styles.done : ''}`}
                  onClick={() => toggleHabit(habit, !isCompleted(habit._id))}
                >
                  {isCompleted(habit._id) ? '✓ Completed' : 'Mark Complete'}
                </button>
              ) : (
                <div className={styles.rangeInput}>
                  <input
                    ...
                    onBlur={(e) => toggleHabit(habit, Number(e.target.value) >= habit.target, Number(e.target.value))}
                  />
                </div>
              )}
            </Card>
